...
async def LLM_Get_Ans(prefs, weather, events, query):
    system_instruction = """Системная инструкция:      Общайся на русском. Ты - помощник для поиска мероприятия для отдыха. 
        Тебе будет дана информация о предпочтениях человека, погоде в его месте нахождения         и проходящих там событиях, мероприятиях и интересных местах в формате JSON. 
